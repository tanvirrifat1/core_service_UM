...
const getMyCourseSchedules = catchAsync(async (req: Request, res: Response) => {
  ...
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: 'CourseSchedules data fetched successfully',
    data: result,
  });
});

const getAcademicInfo = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
  const result = await StudentService.getAcademicInfo(user.userId);
  // console.log(result, 'result');
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
